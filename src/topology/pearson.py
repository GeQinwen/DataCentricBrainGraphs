...
(1) computing functional connectivity (Pearson correlation),
...
Example
-------
python src/topology/pearson.py \
  --root data/rs_100/rs_100_pearson \
  --name HCPGender \
...
class BrainConnectomePearsonFC(InMemoryDataset):
    """
    Creates a PyG dataset where:
      - x: N×N Pearson correlation matrix (diagonal zeroed)
    ...
    def _process_one(self, sid: str, behavioral_df: pd.DataFrame) -> Optional[Data]:
        try:
            ...
            fc = conn.fit_transform([ts])[0].astype(np.float32)  # [N, N]
...
def main():
    p = argparse.ArgumentParser(description="Build FC PyG dataset from time series (Pearson correlation)")
