...
@fill_doc
class KendallConnectivityMeasure(BaseEstimator, TransformerMixin):
    ...
    def _fit_transform(
        ...
        if self.kind == "correlation":
            connectivities = []
            for x in X:
                x_std = signal_re.standardize_signal(x, detrend=False, standardize=self.standardize)
                corr_matrix = np.zeros((x_std.shape[1], x_std.shape[1]))
                for i in range(x_std.shape[1]):
                    for j in range(i, x_std.shape[1]):
                        corr_matrix[i, j], _ = kendalltau(x_std[:, i], x_std[:, j])
                        corr_matrix[j, i] = corr_matrix[i, j]
                connectivities.append(corr_matrix)
        else:
        ...
        if do_transform:
            ...
            if self.vectorize:
                ...
                if confounds is not None:
                    connectivities = signal_re.clean(
